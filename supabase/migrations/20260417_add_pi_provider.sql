-- Add 'pi' to the provider CHECK constraint on daily_snapshots
alter table daily_snapshots
drop constraint if exists daily_snapshots_provider_check;

alter table daily_snapshots
add constraint daily_snapshots_provider_check
check (provider in ('claude', 'codex', 'opencode', 'pi'));

-- Update sync_device_snapshots to accept 'pi' as a valid provider
...
begin
  ...
  if p_provider not in ('claude', 'codex', 'opencode', 'pi') then
